...
with open("rpt_stop_details_202308080948.sql", 'r') as file:
  print('File caricato')
  cmds = file.read().split(';')
  print(f'File letto, sono {len(cmds)} comandi')

  for cmd_idx, cmd in enumerate(cmds):
    if not cmd_idx % 1000: print(f'\rSto parsando la riga {cmd_idx}/{len(cmds)}', end='')
    if not cmd: continue
    cmd = cmd.strip()
...
print()
media_delay = sum([rec['delay'] for rec in tables['rpt_stop_details']['records'] if rec['delay']]) / len(tables['rpt_stop_details']['records'])
print(table, media_delay)
